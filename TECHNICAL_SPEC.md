...
This PowerShell script provides comprehensive network performance measurement capabilities using only built-in Windows 10/11 tools. It requires no administrative privileges and is designed to be easily customizable.

## Measured Metrics

### 1. Packet Loss
- **Method**: ICMP Echo Request/Reply (ping)
- **Tool**: `Test-Connection` cmdlet
...
- **Calculation**: `Average(|ResponseTime - MeanLatency|)` for all samples
- **Unit**: Milliseconds (ms)
...
- **Tool**: `System.Net.Dns.GetHostAddresses()`
- **Unit**: Milliseconds (ms)
...
- **Tool**: `System.Net.Sockets.TcpClient`
- **Measurements**:
...
- All operations use standard .NET and PowerShell APIs
